...
type FutureMotherWelcomeStepProps = {
  onBack: () => void;
  onContinue: () => void;
};

export function FutureMotherWelcomeStep({ onBack, onContinue }: FutureMotherWelcomeStepProps) {
  return (
    <main className="min-h-dvh overflow-hidden bg-background text-text">
      <div className="mx-auto flex min-h-dvh w-full max-w-107.5 flex-col px-6 pb-7 pt-6 md:max-w-[56rem] md:px-10 md:pb-10 md:pt-8">
        ...
        <section className="mx-auto flex flex-1 max-w-[34rem] flex-col items-center justify-center text-center">
          ...
          <p className="maia-auth-mobile-form mt-10 max-w-80 text-lg leading-7 text-text md:mt-8 md:max-w-[28rem] md:text-xl md:leading-8">
            Que lindo saber que você está planejando a maternidade!
          </p>
        </section>
